...
public Action CommandBhopCheckV2(int client, int args)
{
	if (args == 0)
	{
		...
		{
			PrintBhopCheckToChat_V2(client, client);
		}
		return Plugin_Handled;
	}
	...
	if (targetCount >= 2)
	{
		...
		for (int i = 0; i < targetCount; i++)
		{
			...
			{
				PrintBhopCheckToConsole_V2(client, targetList[i]);
			}
		}
	}
	else
	{
		if (GOKZ_AC_GetSampleSizeV2(targetList[0]) == 0)
		{
			if (targetList[0] == client)
			{
				GOKZ_PrintToChat(client, true, "{orchid}[FKZ AC] {default}%t", "Not Enough Bhops (Self)");
		...
		{
			PrintBhopCheckToChat_V2(client, targetList[0]);
		}
	}
